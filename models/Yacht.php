...
class Yacht{
    ...
    public static function find($id){
        $sql = "SELECT yachts.*, users.name as 'owner', brands.name as 'brand' FROM yachts INNER JOIN users ON yachts.id_users = users.id INNER JOIN brands ON yachts.id_brands =brands.id;";
        $conexion = new conexion();
        $conexion->conect();
        $resultado = $conexion->query($sql);
        while ($fila = $resultado->fetch_assoc()) {
